...
pub struct RequestParams {
    ...
    /// User-defined custom auto models referenced by the current selection. Mirrors
    /// `custom_model_providers`: the selected model's `config_key` indexes into this
    /// registry. `None` when no custom auto is selected.
    pub custom_auto_models: Option<warp_multi_agent_api::request::settings::CustomAutoModels>,
...
impl RequestParams {
    ...
    pub fn new(
        ...
        let custom_model_providers = FeatureFlag::CustomInferenceEndpoints
            ...
            .flatten();
        // Build the custom-auto registry for the current base/coding selection.
        // Returns `None` unless a custom auto is selected (and the flag is on).
        let custom_auto_models = LLMPreferences::as_ref(app).custom_auto_models_for_request(
