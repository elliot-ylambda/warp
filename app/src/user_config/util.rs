...
/// Parses a `model_configs/*.yaml` file into custom auto models, returning a
/// per-file `Result` so callers can surface parse/validation errors while still
/// loading other files (inv. 10). Non-YAML files are ignored (`None`).
pub(super) fn parse_model_config_dir_entry(
    item: &DirEntry,
) -> Option<Result<Vec<CustomAutoModel>, ModelConfigError>> {
    let file_name = get_file_name(item)?;
    if !is_config_file(&file_name) {
        return None;
    }
    let make_error = |error_message: String| ModelConfigError {
    ...
    Some(parse_model_configs_yaml(&contents).map_err(make_error))
}
