...
/// terminal view on the GUI side). The shared `AgentStreamDriver` and
...
Extract this into a backend-neutral `AgentStreamDriver` (new module under `app/src/ai/blocklist/`, no terminal/GUI deps) that owns:
...
**`BlocklistAIController` is refactored to call the driver**, so there is a *single* send/stream code path used by both surfaces — not a parallel copy. That is the point of the seam: the TUI gets the controller's canonical streaming/dispatch logic without inheriting the terminal model, the action/input/context models, or the agent-view controller. `CoreTuiModel` calls the same driver with local hooks.

The driver keys history by the owner id (`AgentSessionOwnerId::entity_id()`), passing through to the existing `BlocklistAIHistoryModel` APIs — **no wholesale rename of the history model is required** for the prototype.
...
pub struct CoreTuiModel {
    ...
    /// into history by the `AgentStreamDriver`; `None` when idle.
...
impl CoreTuiModel {
    ...
    /// flight. Delegates the send/stream to the shared `AgentStreamDriver` (§2).
    pub fn send_prompt(
        &mut self,
        prompt: String,
        ctx: &mut ModelContext<Self>,
...
4. Hand the request to the `AgentStreamDriver` with local hooks (`SkillPathOrigin::Local`, no shared-session forwarder, no action sink). The driver creates the `ResponseStream`, appends the user exchange, marks the conversation `InProgress`, subscribes, and folds events into history — including streamed agent text via `ClientActions` (`AddMessagesToTask` / `AppendToMessageContent` carrying `AgentOutput`), which arrives even with tools disabled.
5. Record the returned `ResponseStream` as `in_flight`; clear it and emit `RequestFinished` when the driver reports the stream finished.

The follow-up reuses `active_conversation_id`, so step 3 sends the prior `ConversationData` (tasks + server token), giving the second request the first request's context. Because the fold is the driver's — identical to the GUI controller's — the TUI and GUI cannot drift.

### 6. No client tools in phase one (and why we don't hold an action model)
Pass `supported_tools_override: Some(vec![])` so the server advertises no client tools. This scopes the first milestone to **text exchange and follow-up continuity**. Streamed agent *text* still arrives via `ClientActions` (`AddMessagesToTask` / `AppendToMessageContent`) and is folded into history by the driver (§5); that path needs no action model.
...
Instead, the driver exposes a `ClientActions` **action sink** (§2): today the GUI controller plugs in its `BlocklistAIActionModel`; the TUI plugs in nothing. Nothing in the shared send/stream path hard-codes "no actions" assumptions.
...
- Hits the **real** agent backend, authenticated with the warp-dev credential (no auth flow is built in the prototype). The shared `AgentStreamDriver` (§2) drives the live `ResponseStream`; no fake stream is emitted.
...
### Driver-parity regression test
After `BlocklistAIController` is refactored onto the shared `AgentStreamDriver`, the existing controller / Agent Mode tests must still pass unchanged — proving the GUI send/stream path is behavior-preserving. Add direct coverage that the driver folds `Init` / `ClientActions` / `Finished` / error events into `BlocklistAIHistoryModel` correctly given an owner id + conversation id.
...
- **No `BlocklistAIActionModel` / tool execution.** Pass empty supported tools. Leave a seam (the driver's action sink); don't build the action layer. See §6 for why holding an action model now would be premature and the planned path for tools.
- **No wholesale rename** of `BlocklistAIHistoryModel`'s `terminal_view_id`-named APIs/events. Not needed for the prototype: the shared driver passes the owner id through to the existing APIs. Such a rename can be a later cleanup.

## Sequencing
1. **Shared stream driver** — introduce `AgentSessionOwnerId` and extract the `AgentStreamDriver` (§2) from `BlocklistAIController`, refactoring the controller onto it (with its terminal hooks). Behavior-preserving for the GUI. (Driver-parity regression test.)
...
- **`AgentSessionOwnerId`:** resolved — a newtype over the owning view's `EntityId`, defined near the agent conversation/history types (per-view, not per-model, to support future sub-views). It is the owner key the shared `AgentStreamDriver` and `CoreTuiModel` use; it converts to the `EntityId` the existing history APIs take, so no history rename is needed.
- **Agent logic reuse (driver vs. controller):** resolved — extract a shared `AgentStreamDriver` (Option B) and refactor `BlocklistAIController` onto it; the TUI reuses that single path with local hooks rather than holding the controller / action model or hand-rolling its own send/stream logic.
...
- **Auth + server for the test:** resolved — the prototype builds no auth flow; the end-to-end test reuses the warp-dev credential and exercises the real server through the shared `AgentStreamDriver` (§2). The test is network-dependent by design.
