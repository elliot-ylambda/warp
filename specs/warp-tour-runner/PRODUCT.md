...
13. At tour end (completed, or user chose to end), the runner prints the existing cleanup copy and asks whether to close tour-created panes/tabs. If yes, it closes exactly the recorded tour-created IDs (the tour pane, any settings splits created by stops, and the agent tab if one was created) through normal Warp close behavior and tells the user that Warp's standard close confirmations may appear. If a close fails or is cancelled, the runner reports exactly what is still open. If no, it leaves them and says so.
13a. Settings-backed demonstrations never replace the visible terminal: when a settings page or the keybindings page is opened with a pane target (as every tour open does), it opens as a split below the targeted pane instead of a new Settings tab, so the anchor terminal stays visible. Without a pane target, `surface.settings.open` keeps its existing new-tab behavior. The runner records any pane such an open creates and treats it as tour-created state (see 13).
...
20. `warpctrl tour finish [--tour-pane <id> ...] [--tour-tab <id> ...] [--restore-theme <saved-state-json>]` emits the existing cleanup copy, restores theme state when provided, closes exactly the given tour-created IDs through normal Warp close behavior, and returns per-step results. `--tour-pane` is repeatable so settings splits reported via `created_pane_ids` can be closed alongside the tour pane. It never infers IDs to close; only explicitly passed IDs are touched. (`finish` rather than `cleanup` because `tour cleanup` already names the copy-printing command.)
...
26. The Command Palette offers **"Take the Warp Tour"** (binding `workspace:start_warp_tour`) in builds with the Warp Control feature enabled. Selecting it executes the self-guided tour in the active terminal session with no agent involvement — zero model calls, zero request cost.
27. The entry invokes the running Warp executable in hidden control mode directly, so it works even when the `warpctrl` wrapper is not installed. When Scripting is disabled, the launched tour prints the standard enablement guidance (invariant 1) rather than failing silently.
28. The entry is a normal editable binding: it appears in keybinding settings, can be assigned a shortcut, and is hidden when the Warp Control feature is not compiled in.
